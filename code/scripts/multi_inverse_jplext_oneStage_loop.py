...
I_dif = np.zeros((N_cams, *pixels), dtype=float_reg)
_,_ = scene_rr.render(cuda_paths,I_diff=I_dif)
del(cuda_paths)
print("compiled")
...
if tensorboard:
    tb = TensorBoardWrapper(I_gt, beta_gt, title= datetime.now().strftime("%d%m-%H%M-%S")+f"_multi_Nr={resample_freq}_ss={step_size:.2e}")
...
I_opt, total_grad = scene_rr.render(cuda_paths, I_diff=I_gt, to_print=False)
I_dif = I_opt - I_gt
...
# for iter in range(iterations):
    ...
    I_opt = scene_rr.render(cuda_paths, I_diff=None, to_print=False)
    I_dif = I_opt - I_gt
    # differentiable forward model
    start = time()
    I_opt, total_grad = scene_rr.render(cuda_paths, I_diff=I_dif, to_print=False)
    total_grad *= (ps*ps)
    end = time()
    print(f"rendering took: {end-start}")

    dif = I_dif.reshape(1,1,1, N_cams, *scene_rr.pixels_shape)
