...
downscale = 4
sun_intensity = 1e7 / (downscale*downscale)
...
Ps = airmspi_data["Ps"]#[:3]
resolutions = airmspi_data["resolution"]#[:3]
I_gt = airmspi_data["images"]#[:3]
camera_array_list_fromdata = airmspi_data["camera_array_list"]#[:1]
N_cams = len(ts)
cameras = []
...
for iter in range(iterations):
    ...
    print(f"loss={loss}, Np={Np:.2e}, beta_mean={beta_opt[cloud_mask].mean()}, beta_max={beta_opt.max()}, counter={non_min_couter}")
    ...
    start = time()
    I_opt, total_grad = scene_airmspi.render(cuda_paths, I_gt=I_gt_pad)
    if iter % 10 == 0:
        visual.plot_images_airmspi(I_opt, resolutions, f"Iter {iter}: GPU AirMSPI", downscale)
        plt.show()
        visual.scatter_plot_comparison(I_opt[3], I_gt_pad[3], "third image")
        plt.show()
    ...
    dif = (I_opt - I_gt_pad).reshape(1,1,1, N_cams, *scene_airmspi.pixels_shape_downscaled)
    grad_norm = np.linalg.norm(total_grad)
    print(f"I_gt_mean:{I_gt_pad[I_gt_pad!=0].mean()}, I_opt_mean:{I_opt[I_opt!=0].mean()}, grad_norm:{grad_norm}")
    # updating beta
