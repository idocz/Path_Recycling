import os, sys
my_lib_path = os.path.abspath('./')
sys.path.append(my_lib_path)
from os.path import join
from scene_seed_NEgrad import *
from camera import *
from utils import *
from cuda_utils import *
from classes.optimizer import *
from tqdm import tqdm
cuda.select_device(0)

exp_name = "0503-0955-58_smoke_Nr=10_ss=2.00e+11"
iterations = range(0,260,10)
mins = [60]
########################
# Atmosphere parameters#
########################
sun_angles = np.array([160, 0]) * (np.pi / 180)

#####################
# Volume parameters #
#####################
# construct betas
cloud_gt = loadmat(join("data", "smoke.mat"))["data"] * 10
cloud_gt = np.ascontiguousarray(np.rot90(cloud_gt, axes=(2,1)))
cloud_gt =np.roll(cloud_gt, axis=0, shift=-15)

cloud_gt = cloud_gt.astype(float_reg)
# bounding box
voxel_size_x = 0.02
voxel_size_y = 0.02
voxel_size_z = 0.02
edge_x = voxel_size_x * cloud_gt.shape[0]
edge_y = voxel_size_y * cloud_gt.shape[1]
edge_z = voxel_size_z * cloud_gt.shape[2]
bbox = np.array([[0, edge_x],
                 [0, edge_y],
                 [0, edge_z]])


print(cloud_gt.shape)
print(bbox)

beta_air = 0.04
w0_air = 0.912
w0_cloud = 0.99
g_cloud = 0.5

# Declerations
grid = Grid(bbox, cloud_gt.shape)
volume = Volume(grid, cloud_gt, beta_air, w0_cloud, w0_air)
beta_gt = np.copy(cloud_gt)
#######################
# Cameras declaration #
#######################


ps = 254
pixels = np.array((ps, ps))

N_cams_batch = 9
N_cams = 9

delta_theta = 360/(N_cams*N_cams_batch)
N_cams = 9
height_factor = 1.5
focal_length = 50e-3
sensor_size = np.array((47e-3, 47e-3)) / height_factor
volume_center = (bbox[:, 1] - bbox[:, 0]) / 1.65
R = height_factor * edge_z
cameras = []
all_cameras = []
delta_phi = 360 / (N_cams * N_cams_batch)

for batch in range(N_cams_batch):
    cameras = []
    for cam_ind in range(N_cams):
        theta = np.pi / 2
        phi_ind = N_cams * batch + cam_ind
        phi = (-(N_cams // 2) + phi_ind) * delta_phi
        phi_rad = phi * (np.pi / 180)
        t = R * theta_phi_to_direction(theta, phi_rad) + volume_center
        t[2] -= 0.55
        euler_angles = np.array((90, 0, phi - 90))
        camera = Camera(t, euler_angles, focal_length, sensor_size, pixels)
        cameras.append(camera)
    all_cameras.append(cameras)

# Simulation parameters
Np_gt = int(5e6)
rr_depth = 20
rr_stop_prob = 0.05
scene_seed = SceneSeed(volume, cameras, sun_angles, g_cloud, rr_depth, rr_stop_prob)
scene_seed.init_cuda_param(Np_gt, init=True)
# creating exp dir in checkpoints
output_dir = join("checkpoints", exp_name, "experiments", "gifs")
if not os.path.exists(output_dir):
    os.makedirs(output_dir)


betas = [ np.load(join("checkpoints", exp_name, "data", f"opt_{iteration}.npz"))["betas"]
          for iteration in range(0,260,10) ]
rounds = len(betas) // 3

img_list = []
total_batch = rounds *  N_cams_batch
for round in range(rounds+1):
    for batch in tqdm(range(N_cams_batch)):
        if round < rounds:
            beta_conti_ind = len(betas)*((round*N_cams_batch + batch)/total_batch)
            beta_int = int(beta_conti_ind)
            print(beta_int)
            beta_lambda = beta_conti_ind - beta_int
            beta1 = betas[beta_int]
            if beta_int < len(betas) - 1:
                beta2 = betas[beta_int+1]
                cloud_opt = beta1 + beta_lambda * (beta2 - beta1)
            else:
                cloud_opt = beta1
        else:
            cloud_opt = betas[-1]
        scene_seed.set_cameras(all_cameras[batch])
        # opt
        # exp_cloud_name = join("checkpoints", exp_name, "data", f"opt_{iterations[batch]}.npz")
        # cloud_opt = np.load(exp_cloud_name)["betas"]
        volume.beta_cloud = cloud_opt
        cuda_paths = scene_seed.build_paths_list(Np_gt)
        I_opt = scene_seed.render()
        # gt
        volume.beta_cloud = cloud_gt
        cuda_paths = scene_seed.build_paths_list(Np_gt)
        I_gt = scene_seed.render()
        img_concat = [np.hstack([np.rot90(img1),np.rot90(img2)]) for img1, img2 in zip(I_opt,I_gt)]
        img_list.extend(img_concat)

print("saving gif...")
output_name = join(output_dir,f"rtemp.mp4")
# output_name = None
animate(img_list, interval=100, output_name=output_name, repeat_delay=0)


