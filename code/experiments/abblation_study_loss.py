...
line = plt.loglog(ts_reg, values_reg, linewidth=linewidth)
plt.loglog(ts_rec, values_rec, label="scattering tomography",color="#F39C12", linewidth=linewidth)
plt.grid()
...
plt.loglog(ts_rec_surf, values_rec_surf, "--", label="reflectometry",color="#F39C12", linewidth=linewidth)
plt.loglog(ts_reg_surf, values_reg_surf, "--", color=line[0].get_color(), linewidth=linewidth)
...
plt.savefig(join(output_dir,f"ablation_studies_loss.pdf"), bbox_inches='tight')
plt.show()
