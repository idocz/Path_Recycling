...
line = plt.plot(ts_reg, values_reg)#, linewidth=linewidth)
plt.plot(ts_rec, values_rec, label="scattering tomography",color="#F39C12")#, linewidth=linewidth)
plt.grid()
plt.xlim(5e-3,2)
plt.xticks(fontsize=tick_size)
plt.yticks(fontsize=tick_size)
plt.tight_layout()
plt.savefig(join(output_dir,f"ablation_studies_rel_dist_scattering_SUP.pdf"), bbox_inches='tight')
plt.show()

# reflectometry

plt.figure(figsize=(8,4.5))
plt.plot(ts_rec_surf, values_rec_surf, label="reflectometry",color="#F39C12")#, linewidth=linewidth)
plt.plot(ts_reg_surf, values_reg_surf, color=line[0].get_color())#, linewidth=linewidth)
plt.grid()
plt.tight_layout()
plt.xticks(fontsize=tick_size)
plt.yticks(fontsize=tick_size)
plt.xlim(5e-3,2)
plt.savefig(join(output_dir,f"ablation_studies_rel_dist_reflectometry_SUP.pdf"), bbox_inches='tight')
plt.show()
