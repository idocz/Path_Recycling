...
def render_path(path, betas, beta_air, w0_cloud, w0_air):
    ...
    optical_length = np.zeros((N_cams, N_seg))
    ...
    si = scatter_tensor
    scatter_betas = np.empty(si.shape[1])
    for ind in range(si.shape[1]):
        scatter_betas[ind] = betas[si[0,ind], si[1,ind], si[2,ind]]
    scatter_cont = w0_cloud*(scatter_betas - beta_air) + w0_air*beta_air
    scatter_cont = np.cumprod(scatter_cont).reshape(1,-1)
    res = (scatter_cont * np.exp(-optical_length)) * ISs_mat
    return res
