...
class SceneSeed(object):
    def __init__(self, volume: Volume, cameras, sun_angles, g_cloud, rr_depth, rr_stop_prob):
        ...
        @cuda.jit()
        def render_cuda(beta_cloud, beta_zero, beta_air, g_cloud, w0_cloud, w0_air, sun_direction, bbox, bbox_size, voxel_size, N_cams,
                        ts, Ps, pixel_shape, is_in_medium, rng_states, scatter_sizes, I_total):
            ...
            if tid < rng_states.shape[0] - 1:
                ...
                while True:
                    ...
                    if seg > 0:
                        # angle pdf
                        IS *= cloud_prob_scat * HG_pdf(cos_theta_scatter, g_cloud) + (
                                    1 - cloud_prob_scat) * rayleigh_pdf(cos_theta_scatter)
                        IS /= cloud_prob_scat0 * HG_pdf(cos_theta_scatter, g_cloud) + (
        ...
        def render_differentiable_cuda(beta_cloud, beta_zero, beta_air, g_cloud, w0_cloud, w0_air, sun_direction, bbox, bbox_size, voxel_size,
                         rng_states, cloud_mask, total_grad):
            tid = cuda.grid(1)
    ...
    def build_paths_list(self, Np, init=False, to_print=False):
    ...
    def render(self, I_diff=None, to_print=False):
        # east declerations
